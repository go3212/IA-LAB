...
public class RandomSolutionStateUnponderated extends State
{
    public RandomSolutionStateUnponderated(Usuarios users)
    {
        super(users);
                ...
                while (car.HasCapacity() && (pickups.size() > 0 || dropoffs.size() > 0 ))
                {
                    ...
                    if (car.RouteDistanceMeters() >= State.ROUTE_MAX_DISTANCE_IN_METERS)
                    {
                        do
                        {
                            ...
                            pickups.add(last);
                            dropoffs.add(last);
                        } while (car.RouteDistanceMeters() + car.DistanceTo(car.GetOwner()) >= State.ROUTE_MAX_DISTANCE_IN_METERS);
                        break;
                    }
                }
            }
